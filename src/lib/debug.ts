...
class LogAgent {
  ...
  private loadFilters() {
    ...
    const filter = localStorage?.getItem(`log_agent_filter_${this.appName}`) ?? '*'

    this.enabledNamespaces = filter.split(',').map((f) => new RegExp('^' + f.replace(/\*/g, '.*') + '$'))
  }
  ...
  private log(namespace: string, level: LogLevel, message: any[]) {
    ...
    console.groupCollapsed(
      `%c[${record.app}] %c[${namespace}] %c${level.toUpperCase()} %c${new Date(record.timestamp).toISOString()}`,
