    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-10">
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Somente o autor do evento pode editar ou excluir este vídeo.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-4 rounded-xl border bg-card p-5 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            <Textarea id="summary" value={summary} onChange={(event) => setSummary(event.target.value)} className="min-h-[140px]" />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea id="content-warning" value={contentWarning} onChange={(event) => setContentWarning(event.target.value)} className="min-h-[80px]" />
          </div>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            {thumbnail ? <Image src={thumbnail} alt="Thumbnail" width={288} className="aspect-video w-full rounded-lg border object-cover" /> : null}
            <div className="mt-4">
              <ButtonUploadThumb setUrl={(nextUrl) => nextUrl && setThumbnail(nextUrl)} url={thumbnail} accept={{ "image/*": [] }}>
                <Button variant="outline" className="w-full">Alterar thumbnail</Button>
              </ButtonUploadThumb>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Digite <strong>DELETAR</strong> para confirmar que esta ação é irreversível.</p>
            <Input className="mt-4" value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} placeholder="DELETAR" />
            <ButtonWithLoader className="mt-4 w-full" variant="destructive" isLoading={isDeleting} disabled={deleteConfirmation !== "DELETAR"} onClick={handleDelete}>
