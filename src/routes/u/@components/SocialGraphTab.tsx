...
export function SocialGraphTab({ pubkey, profileTitle, profilePicture }: SocialGraphTabProps) {
  ...
  const graphQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const metadataEvents = visiblePubkeys.length
        ? await fetchEventsCached(ndk!, { kinds: [NDKKind.Metadata], authors: visiblePubkeys, limit: visiblePubkeys.length }, { mode: "parallel" })
        : new Set<NDKEvent>();
  ...
  const mutuals = useMemo(
    () => follows.filter((follow) => followers.includes(follow)).length,
    [followers, follows],
  );
    ...
    return (
      <Card>
        <div className="flex flex-col items-center justify-center px-6 py-16 text-center text-muted-foreground">
          ...
          <p className="text-sm">Nenhum dado público de rede encontrado para este perfil.</p>
        </div>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-3">
        <GraphMetric label="Seguindo" value={follows.length} />
        <GraphMetric label="Seguidores encontrados" value={followers.length} />
        <GraphMetric label="Conexões mútuas" value={mutuals} />
      </div>
          ...
          <div className="border-b border-border/70 p-5 lg:border-b-0 lg:border-r">
            ...
            <div className="mt-5 rounded-lg border bg-muted/30 p-4 text-sm text-muted-foreground">
              <div className="flex items-center gap-2 text-foreground">
                ...
                <span className="font-medium">Nós conectados</span>
              </div>
              <p className="mt-2">
                Exibindo {nodes.length} perfis públicos relacionados por follows e seguidores recentes.
              </p>
            </div>
          </div>
          ...
          <div className="p-5">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {nodes.map((node) => {
                ...
                const relation = isMutual ? "Mútuo" : follows.includes(node.pubkey) ? "Segue" : "Seguidor";
