  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto p-6 space-y-6">
      <Card className="rounded-2xl shadow-md">
        <CardHeader>
          <CardTitle>{t('user_edit_title')}</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col space-y-2">
              ...
              />
              {form.banner && <img src={form.banner} alt="banner" className="h-40 w-full object-cover rounded-xl" />}
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder={t('user_edit_about_placeholder')}
                value={form.about}
            ...
            {/* Lightning */}
            <div>
              <Label>{t('user_edit_lightning')}</Label>
              <Input
                placeholder="ex: fulano@walletofsatoshi.com"
                value={form.lnAddress}
                onChange={handleChange('lnAddress')}
              />
            </div>

            <ButtonWithLoader type="submit" className="w-full" isLoading={mut.isPending}>
              Salvar
            </ButtonWithLoader>
          </form>
        </CardContent>
