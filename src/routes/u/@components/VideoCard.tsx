...
export function VideoCard({ event }: VideoCardProps) {
  ...
  const title = tagData.title || imgTitle || t('user_video_untitled')
  ...
  return (
    ...
    >
      <Card className="h-full border-border/40 bg-card overflow-hidden hover:shadow-xl transition-all dark:hover:border-primary/30">
        {/* Container da Thumbnail com Aspect Ratio mantido */}
        <div className="relative aspect-video bg-muted overflow-hidden">
          {optimized && !error ? (
            <>
              {/* Skeleton de carregamento */}
              {loading && <div className="absolute inset-0 z-10 animate-pulse bg-neutral-800" />}
          ...
          {/* Badge de Duração */}
          {duration && (
            <span className="absolute bottom-2 right-2 z-30 bg-black/80 text-white text-[10px] px-1.5 py-0.5 rounded-sm font-mono font-bold backdrop-blur-sm">
              {duration}
            </span>
          )}
        </div>
        ...
        <CardHeader className="p-3 pb-0">
          <h3 className="font-semibold text-sm sm:text-base line-clamp-2 leading-tight group-hover:text-primary transition-colors min-h-[2.5rem]">
            {title}
          </h3>
        </CardHeader>
        ...
        <CardContent className="p-3 pt-1.5">
          <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">
            {tagData.summary || t('user_video_no_description')}
          </p>
        </CardContent>

        <CardFooter className="p-3 pt-0 text-[10px] uppercase tracking-wider text-muted-foreground/70 flex justify-between items-center">
          ...
          {isNSFW && (
            <span className="text-destructive font-bold border border-destructive/30 px-1 rounded-[2px]">NSFW</span>
          )}
        </CardFooter>
      </Card>
    </Link>
