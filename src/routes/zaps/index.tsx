      ...
      <CardContent className="flex flex-col items-center justify-center gap-4 py-12 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="max-w-xl text-sm text-muted-foreground">
            Quando recebimentos ou apoios com NIP-57 estiverem visíveis nos relays consultados, esta tela mostrará atividade real sem inventar valores monetários.
          </p>
        </div>
          ...
          <CardContent className="flex flex-col items-center justify-center gap-4 py-12 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="max-w-lg text-sm text-muted-foreground">
                A tela usa dados reais dos seus recebimentos e apoios identificáveis via NIP-57. Sem uma sessão ativa, não há como calcular suas métricas pessoais.
              </p>
            </div>
      ...
      >
        <Card>
          <CardContent className="flex flex-col items-center justify-center gap-4 py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">Tente novamente. Os botões de Zap existentes permanecem intactos.</p>
            <Button variant="outline" onClick={() => void statsQuery.refetch()}>Tentar novamente</Button>
          </CardContent>
        </Card>
      </AppShell>
    )
    ...
    <>
        ...
        <CardContent className="space-y-3">
          {supportersQuery.supporters.length > 0 ? (
            supportersQuery.supporters.map((supporter, index) => {
              ...
              return (
                <div key={supporter.pubkey} className="flex items-center gap-3 rounded-2xl border border-border/60 bg-card/50 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-foreground">{profile?.displayName || profile?.name || trimPubkey(supporter.pubkey)}</p>
        ...
        <CardContent className="space-y-3">
          {!goals.hasGoals ? (
            <div className="rounded-2xl border border-dashed border-border/70 p-4">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Ainda não encontramos uma estrutura real de metas/campanhas no projeto atual.</p>
            </div>
          ) : null}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-muted-foreground">
          <div className="flex items-start gap-2"><Sparkles className="mt-0.5 size-4 text-[oklch(var(--lightning))]" />Recompense bons criadores.</div>
          <div className="flex items-start gap-2"><Sparkles className="mt-0.5 size-4 text-[oklch(var(--lightning))]" />Adicione mensagens para contexto.</div>
          <div className="flex items-start gap-2"><Sparkles className="mt-0.5 size-4 text-[oklch(var(--lightning))]" />Use metas para engajar sua audiência.</div>
          <div className="flex items-start gap-2"><Sparkles className="mt-0.5 size-4 text-[oklch(var(--lightning))]" />Zaps reforçam a descoberta de conteúdo.</div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Os botões de Zap já existentes continuam sendo o fluxo principal para pagar.</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard title="Recebidos" value={formatSats(data?.received30d ?? null)} description="Últimos 30 dias, quando calculável." icon={Wallet} tone="zap" />
        <MetricCard title="Enviados" value={formatSats(data?.sent30d ?? null)} description="Detectados por receipts visíveis no período." icon={HeartHandshake} tone="default" />
        <MetricCard title="Criadores apoiados" value={data?.supportedCreatorsCount == null ? <UnavailableValue reason="Nenhum criador identificado no período." /> : String(data.supportedCreatorsCount)} description="Pubkeys distintas apoiadas." icon={HandCoins} tone="success" />
        <MetricCard title="Taxa média por zap" value={formatSats(data?.averageZapSats ?? null)} description="Média real no período disponível." icon={BarChart3} tone="relay" />
      </div>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as ZapTab)} className="w-full">
        <TabsList className="mb-6 flex h-auto w-full flex-wrap justify-start gap-2 rounded-2xl border border-border/70 bg-card/70 p-2">
          <TabsTrigger value="received" className="rounded-xl data-[state=active]:border-b-2 data-[state=active]:border-[oklch(var(--lightning))]">Recebidos</TabsTrigger>
          <TabsTrigger value="sent" className="rounded-xl data-[state=active]:border-b-2 data-[state=active]:border-[oklch(var(--lightning))]">Enviados</TabsTrigger>
          <TabsTrigger value="goals" className="rounded-xl data-[state=active]:border-b-2 data-[state=active]:border-[oklch(var(--lightning))]">Metas</TabsTrigger>
          <TabsTrigger value="campaigns" className="rounded-xl data-[state=active]:border-b-2 data-[state=active]:border-[oklch(var(--lightning))]">Campanhas</TabsTrigger>
        </TabsList>
          ...
          <CardHeader>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant={timeRange === 'seven' ? 'zap' : 'glass'} size="sm" onClick={() => setTimeRange('seven')}>7 dias</Button>
                <Button variant={timeRange === 'thirty' ? 'zap' : 'glass'} size="sm" onClick={() => setTimeRange('thirty')}>30 dias</Button>
                <Button variant={timeRange === 'ninety' ? 'zap' : 'glass'} size="sm" onClick={() => setTimeRange('ninety')}>90 dias</Button>
              </div>
            </div>
          </CardHeader>
          <CardContent>
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Area type="monotone" dataKey="received" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.18} name="Recebidos" />
                    <Area type="monotone" dataKey="sent" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.12} name="Enviados" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ) : (
          ...
          <Card>
            ...
            <CardContent>
              {bestVideo ? (
                ...
                <p className="text-sm text-muted-foreground"><UnavailableValue reason="Nenhum vídeo com zaps recebidos identificado." /></p>
              )}
            </CardContent>
          </Card>
          <Card>
              ...
              {topSupporter ? (
                <div className="flex items-center gap-3">
                  <Avatar>
                    ...
                    <AvatarFallback>{(topSupporterProfile?.name || topSupporter.pubkey).slice(0, 1).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="text-sm font-medium text-foreground">{topSupporterProfile?.displayName || topSupporterProfile?.name || trimPubkey(topSupporter.pubkey)}</p>
                    <p className="text-xs text-muted-foreground">{formatSats(topSupporter.amountSats)}</p>
                  </div>
                </div>
              ) : (
                <p className="text-sm text-muted-foreground"><UnavailableValue reason="Nenhum apoiador identificado no período." /></p>
              )}
            </CardContent>
          </Card>
          <Card>
        ...
        <TabsContent value="goals" className="mt-0 space-y-4">
          <Card>
            <CardContent className="py-10 text-center">
              ...
              <p className="font-medium text-foreground">Crie sua primeira meta de apoio</p>
              <p className="mt-1 text-sm text-muted-foreground">Não encontramos uma modelagem real de metas nesta base atual.</p>
            </CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="campaigns" className="mt-0 space-y-4">
          <Card>
            <CardContent className="py-10 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Sem implementação real encontrada para campanhas de Zap neste projeto.</p>
            </CardContent>
          </Card>
        </TabsContent>
                  ...
                  {filteredActivity.map((item) => (
                    <tr key={item.id} className="border-b border-border/50 align-top last:border-b-0">
                      <td className="px-4 py-4">
                        <StatusBadge tone={item.direction === 'received' ? 'warning' : 'partial'}>{item.direction === 'received' ? 'Recebido' : 'Enviado'}</StatusBadge>
                      </td>
                      <td className="px-4 py-4 font-mono text-xs text-muted-foreground">{trimPubkey(item.direction === 'received' ? item.senderPubkey : item.recipientPubkey)}</td>
                      <td className="px-4 py-4">
                        <p className="text-sm font-medium text-foreground">{item.targetLabel}</p>
                        {item.targetRef ? <p className="max-w-[260px] truncate text-xs text-muted-foreground">{item.targetRef}</p> : null}
                      </td>
                      ...
                      <td className="px-4 py-4"><StatusBadge tone={statusConfig(item.status).tone}>{statusConfig(item.status).label}</StatusBadge></td>
                    </tr>
                  ))}
                </tbody>
              </table>
