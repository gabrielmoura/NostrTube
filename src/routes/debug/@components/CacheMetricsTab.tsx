...
import { getAllStoreSizes, getCacheMetrics, getReadRate, getStorageQuota, getWriteRate } from "@/features/debug/services/metrics.service.ts";
...
export function CacheMetricsTab() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [m, q, sizes] = await Promise.all([
        getCacheMetrics(),
        getStorageQuota(),
        getAllStoreSizes(),
      ]);
  ...
  const metricsList: MetricsDisplay[] = metrics
    ? [
      ...
      { label: "Entradas LNURL", value: metrics.lnurlCount.toLocaleString() },
      {
        ...
        badge: metrics.duplicateCount > 0
  ...
  function pieSvg(segments: { kind: number; count: number }[], total: number) {
    if (total === 0) return null;
    const colors = ["#6366f1", "#8b5cf6", "#a855f7", "#d946ef", "#ec4899", "#f43f5e", "#f97316", "#eab308", "#22c55e", "#14b8a6", "#06b6d4", "#3b82f6"];
    const cx = 60, cy = 60, r = 50;
    let cumulative = 0;
    const slices = segments.slice(0, 12).map((seg, i) => {
      ...
      return (
        ...
        >
          <title>{`kind ${seg.kind}: ${seg.count} (${Math.round((seg.count / total) * 100)}%)`}</title>
        </path>
      );
    });
    if (segments.length > 12) {
      const rest = segments.slice(12).reduce((s, e) => s + e.count, 0);
      if (rest > 0) {
        const angle = (rest / total) * 360;
        const startAngle = cumulative;
        cumulative += angle;
        const startRad = ((startAngle - 90) * Math.PI) / 180;
        const endRad = ((startAngle + angle - 90) * Math.PI) / 180;
        ...
        slices.push(
          <path
            ...
            fill="#a1a1aa"
          >
            <title>{`Outros: ${rest} (${Math.round((rest / total) * 100)}%)`}</title>
          </path>,
        );
      }
    }
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-mono text-zinc-900 dark:text-zinc-100">
                  {quota.usagePercent}% ({Math.round(quota.usage / 1024 / 1024)}MB / {Math.round(quota.quota / 1024 / 1024)}MB)
                </span>
              </div>
          ...
          <CardContent>
            <div className="flex items-start gap-4">
              <div className="flex-shrink-0">
                {metrics && totalByKind > 0 ? pieSvg(metrics.eventsByKind, totalByKind) : null}
              </div>
              ...
              {storeSizes.map((s) => {
                ...
                return (
                  <div key={s.store} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-zinc-700 dark:text-zinc-300 font-mono w-16 text-right">{s.count.toLocaleString()}</span>
                  </div>
                );
              })}
            </div>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {sortedRelays.map((r) => (
                  <tr key={r.relay} className="border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                ...
                {sortedRelays.length === 0 && (
                  <tr>
                    <td colSpan={2} className="py-4 text-center text-zinc-400">Nenhum evento com relay registrado</td>
                  </tr>
                )}
              </tbody>
            </table>
