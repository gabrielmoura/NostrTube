      ...
      <div className="w-full space-y-6">
        <Tabs
          ...
          onValueChange={(nextTab) =>
            navigate({ to: '/', search: { tab: nextTab as 'trending' | 'popular' | 'language' } })
          }
          className="w-full"
        >
...
function VideoFeed({ events, title, isLoading, emptyMessage }: VideoFeedProps) {
  const processedEvents = useMemo(() => {
    ...
    return uniqBy((e) => getTagValues('title', e.tags), events).sort(sortEventsByImages)
  }, [events])
...
function RecentVideos() {
  ...
  return <VideoFeed events={filtered} title={t('Recent Uploads', 'Envios Recentes')} isLoading={events.length === 0} />
}
...
function LanguageVideos() {
  ...
  if (!lang) {
    return (
      <div className="p-5 text-center text-muted-foreground">{t('Language not detected', 'Idioma não detectado.')}</div>
    )
  }
