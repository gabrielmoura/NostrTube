...
export function FeedbackCategorySelect({ value, onChange, disabled = false, errorMessage }: FeedbackCategorySelectProps) {
  return (
    <div className="space-y-3">
      <div role="radiogroup" aria-invalid={Boolean(errorMessage)} aria-describedby={errorMessage ? "feedback-category-error" : undefined} className="grid gap-2 sm:grid-cols-2">
      ...
      {errorMessage ? <p id="feedback-category-error" className="text-sm text-destructive">{errorMessage}</p> : null}
    </div>
  );
}
