...
function getPrimaryLabel(stage: ReturnType<typeof useSubmitFeedback>["stage"], successState: ReturnType<typeof useSubmitFeedback>["successState"]) {
...
export function FeedbackModal({ open, onOpenChange }: FeedbackModalProps) {
      ...
      {submission.successState ? (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border bg-card p-4 text-sm">
            ...
            <p className="mt-2 break-all font-mono text-xs text-muted-foreground">{submission.successState.feedbackId}</p>
            <p className="mt-3 text-xs text-emerald-700 dark:text-emerald-400">{t("feedback.success.feedback_published")}</p>
          </div>

          <div className="rounded-2xl border bg-card p-4 text-sm">
            <p className="font-medium text-foreground">{t("feedback.success.message_id")}</p>
            <p className="mt-2 break-all font-mono text-xs text-muted-foreground">{submission.successState.messageId}</p>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              {...form.register("title")}
            />
            {form.formState.errors.title ? <p id="feedback-title-error" className="text-sm text-destructive">{form.formState.errors.title.message}</p> : null}
          </div>
          ...
          <div className="space-y-2">
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <FeedbackCategorySelect value={field.value} onChange={field.onChange} disabled={isBusy} errorMessage={fieldState.error?.message} />
              )}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p id="feedback-message-help" className="text-xs text-muted-foreground">{t("feedback.fields.message_help_private")}</p>
            {form.formState.errors.message ? <p id="feedback-message-error" className="text-sm text-destructive">{form.formState.errors.message.message}</p> : null}
          </div>
            ...
            <Controller
              ...
              render={({ field }) => (
                <FeedbackZapAmountSelect
                  ...
                  onCustomAmountChange={(value) => form.setValue("customZapAmount", value, { shouldDirty: true, shouldValidate: true })}
                  disabled={isBusy}
          ...
          {submission.stage === "pow" ? (
            <div className="rounded-2xl border bg-card p-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">{t("feedback.progress.pow_attempts", { count: submission.powProgress?.attempts?.toLocaleString() || 0 })}</p>
              </div>
            </div>
          ) : null}
  ...
  const footer = submission.successState ? (
    <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
      <Button type="button" variant="outline" onClick={handleSuccessReset}>{t("feedback.actions.send_another")}</Button>
      <Button type="button" onClick={() => handleCloseRequest(false)}>{t("feedback.actions.close")}</Button>
    </div>
  ) : (
    <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
      <Button type="button" variant="outline" disabled={isBusy} onClick={isBusy ? submission.cancel : () => handleCloseRequest(false)}>
      ...
      <Dialog open={open} onOpenChange={handleCloseRequest}>
        ...
        >
          <DialogHeader className="border-b px-6 py-4">
            <DialogTitle className="flex items-center gap-2"><MessageSquare className="size-5" /> {t("feedback.modal_title")}</DialogTitle>
    ...
    <Drawer open={open} onOpenChange={handleCloseRequest} dismissible={!isBusy}>
      <DrawerContent className="flex max-h-[92svh] flex-col rounded-t-2xl">
        <DrawerHeader className="border-b text-left">
          <DrawerTitle className="flex items-center gap-2"><MessageSquare className="size-5" /> {t("feedback.modal_title")}</DrawerTitle>
