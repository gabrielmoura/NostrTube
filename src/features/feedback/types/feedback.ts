...
const feedbackCategoryValues = FEEDBACK_CATEGORIES.map((category) => category.value) as [
...
export const feedbackFormSchema = z
  .object({
    ...
    email: z
      ...
      .max(160, 'Use no maximo 160 caracteres no e-mail.')
      .optional()
      .or(z.literal('')),
    message: z
      .string()
      .trim()
      .min(20, 'Descreva sua mensagem com pelo menos 20 caracteres.')
      .max(3000, 'Use no maximo 3000 caracteres na mensagem.'),
...
export type FeedbackSubmissionStage =
  | 'idle'
  | 'preparing'
  | 'pow'
  | 'signing'
...
export function resolveFeedbackZapAmount(
  values: Pick<FeedbackFormValues, 'zapPreset' | 'customZapAmount'>,
): number | undefined {
