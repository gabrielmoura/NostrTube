...
import { FEEDBACK_POW_DIFFICULTY, FEEDBACK_PUBLISH_TIMEOUT_MS } from '@/config/feedback.const'
import { getFeedbackCategory } from '@/features/feedback/constants/feedbackCategories'
import {
  buildFeedbackContent,
  buildFeedbackRumorTags,
  collectFeedbackTechnicalDetails,
  getBrowserLanguage,
} from '@/features/feedback/services/feedbackEventService'
...
import { getFeedbackRecipientConfig } from '@/helper/feedback'
...
export function useSubmitFeedback() {
  ...
  const isPowActiveRef = useRef(false)
  ...
  const updatePowProgress = useThrottledCallback(
    (attempts: number) => {
  ...
  const submit = useCallback(
    async (values: FeedbackFormValues) => {
      ...
      try {
        ...
        try {
          ...
          throw new FeedbackFlowError('invalid-recipient', 'Invalid feedback recipient configuration.', {
            cause: error,
          })
        }
        ...
        try {
          ...
          const success: FeedbackSuccessState = {
            ...
            zapMessage:
              zapOutcome.status === 'paid' ? t('feedback.success.sent_with_zap') : t('feedback.success.invoice_ready'),
            publishedRelays,
          }
    ...
    [currentPubkey, ndk, updatePowProgress],
  )

  const isSubmitting = stage === 'preparing' || stage === 'pow' || stage === 'publishing' || stage === 'zap'
