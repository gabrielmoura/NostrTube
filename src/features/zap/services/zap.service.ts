import NDK, { type NDKEvent, type NDKPaymentConfirmationLN, type NDKUser, NDKZapper } from '@nostr-dev-kit/ndk'
...
async function resolveZapRecipient(ndk: NDK, target: ZapTarget): Promise<NDKUser | NDKEvent> {
  ...
  return resolveMessageRecipient(ndk, target.pubkey, target.pubkey).catch((error) => {
    throw new ZapFlowError('recipient-metadata-failed', 'Could not resolve recipient metadata for ZAP.', {
      cause: error,
    })
  })
}
...
export async function startZap({ ndk, target, amountSats, comment }: StartZapParams): Promise<ZapOutcome> {
      ...
      if (typeof window !== 'undefined' && window.webln) {
        try {
          ...
          if (isPaymentCancelled(message)) {
            throw new ZapFlowError('zap-payment-cancelled', 'The Lightning payment was cancelled.', {
              cause: error instanceof Error ? error : undefined,
            })
          }

          throw new ZapFlowError('zap-payment-failed', 'The Lightning payment failed.', {
            cause: error instanceof Error ? error : undefined,
          })
        }
      }
  ...
  try {
    ...
    if (message.includes('no zap method') || message.includes('no zap endpoint') || message.includes('no zap spec')) {
      throw new ZapFlowError('zap-unavailable', 'This recipient does not have a valid Lightning address for ZAP.', {
        cause: error instanceof Error ? error : undefined,
      })
    }

    if (message.includes('invoice') || message.includes('lnurl') || message.includes('payment request')) {
      throw new ZapFlowError('zap-invoice-failed', 'Could not generate the Lightning invoice.', {
        cause: error instanceof Error ? error : undefined,
      })
    }
