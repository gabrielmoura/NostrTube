...
export function useZapSubmission() {
  ...
  const submit = useCallback(async ({ target, amountSats, comment }: SubmitZapParams): Promise<ZapOutcome | null> => {
    if (!currentUser || !ndk) {
      const message = getZapErrorMessage(new ZapFlowError("unauthenticated", "User must be logged in before sending a zap."));
      setStatus("error");
