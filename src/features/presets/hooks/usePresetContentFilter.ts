...
export function usePresetContentFilter() {
  ...
  const isEventHidden = useCallback(
    (event: Pick<NDKEvent, 'id' | 'pubkey'>) => isEventHiddenByPreset(event, activePreset),
    [activePreset],
  )
  const filterEvents = useCallback(
    <T extends Pick<NDKEvent, 'id' | 'pubkey'>>(events: T[]) => filterEventsByPreset(events, activePreset),
    [activePreset],
  )
