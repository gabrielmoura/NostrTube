...
import { AlertCircle, CheckCircle2, ChevronRight, ExternalLink, Film, LinkIcon, PlaySquare, Sparkles, Youtube } from "lucide-react";
    ...
    <div className="overflow-hidden rounded-3xl border border-border/70 bg-card/80 shadow-[0_20px_80px_color-mix(in_oklab,var(--background)_55%,black_20%)] backdrop-blur-xl">
      <div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-center">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-tight">{t("import_youtube_title", "Importar vídeo do YouTube")}</h2>
      ...
      <div className="grid gap-3 border-t border-border/70 bg-secondary/20 p-4 text-sm text-muted-foreground sm:grid-cols-3 sm:px-6">
        <div className="flex items-start gap-2">
          ...
          <span>{t("youtube_import_metadata_only", "Nesta etapa, o vídeo não será baixado nem hospedado no Blossom.")}</span>
        </div>
        <div className="flex items-start gap-2">
          <LinkIcon className="mt-0.5 size-4 shrink-0 text-primary" />
          <span>{t("youtube_import_original_reference", "O evento apontará para a URL original e para a thumbnail pública do YouTube.")}</span>
        </div>
        <div className="flex items-start gap-2">
          <Sparkles className="mt-0.5 size-4 shrink-0 text-primary" />
          <span>{t("youtube_import_phase_one_notice", "Publicação e metadados automáticos do player entram nas próximas fases.")}</span>
        </div>
      </div>
    </div>
  );
    ...
    <form onSubmit={onSubmit} className="rounded-3xl border border-border/70 bg-card/80 p-5 shadow-sm backdrop-blur">
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          />
          <Button type="submit" variant="gradient" size="lg" isLoading={isImporting} disabled={!url.trim() || isImporting}>
            {t("Import", "Importar")}
          </Button>
        </div>
        <div id="youtube-url-hint" className="min-h-5">
          {showInvalidState ? (
            ...
            <p className="text-sm text-muted-foreground">
              {t("youtube_url_empty_state", "Aceita links youtube.com/watch, youtu.be, embed, v, shorts e youtube-nocookie.")}
            </p>
          )}
        </div>
      </div>
    ...
    return (
      <div className="flex aspect-video flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-border/80 bg-secondary/30 p-5 text-center">
        ...
        <p className="max-w-[260px] text-xs text-muted-foreground">
          {importedVideo
            ? t("thumbnail_fallback_hint", "A imagem pública do YouTube não carregou, mas a URL e o ID continuam disponíveis.")
    ...
    <aside className="space-y-5 lg:sticky lg:top-24 lg:h-fit">
      <div className="rounded-3xl border border-border/70 bg-card/80 p-4 shadow-sm backdrop-blur">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{t("import_preview_desc", "Dados detectados antes da publicação.")}</p>
          </div>
          <span
            className={cn(
              "rounded-full border px-3 py-1 text-xs font-medium",
              importedVideo ? "border-primary/30 bg-primary/10 text-primary" : "border-border bg-secondary/50 text-muted-foreground",
            )}
          >
        ...
        {isLoading ? <Skeleton className="aspect-video w-full rounded-2xl" /> : <PreviewThumbnail importedVideo={importedVideo} />}
      ...
      <div className="rounded-2xl border border-primary/20 bg-primary/10 p-4 text-sm text-primary">
        ...
        <p className="mt-1 text-primary/80">
          {t("youtube_import_not_hosted_desc", "A tela prepara metadados e preview. O evento e a publicação serão ativados nas próximas fases.")}
        </p>
      </div>
    </aside>
  );
...
export function YouTubeImportPageContainer() {
  ...
  const handlePublish = async () => {
    ...
    const result = await publish(buildYouTubeVideoDraft({
        ...
        <div className="min-w-0 space-y-6">
          {publishedState ? (
            <div className="rounded-3xl border border-primary/30 bg-primary/10 p-6 shadow-sm">
              <div className="mb-4 flex items-center gap-3 text-primary">
                ...
                <h2 className="text-xl font-semibold">{t("youtube_publish_success_title", "Vídeo do YouTube publicado")}</h2>
              </div>
              <p className="text-sm text-muted-foreground">
                {t("youtube_publish_success_desc", "O evento Nostr foi publicado com a referência para o vídeo original do YouTube.")}
              </p>
          ...
          <div className="rounded-3xl border border-border/70 bg-card/80 p-5 shadow-sm backdrop-blur">
            <div className="mb-5">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {t("youtube_metadata_hint", "Depois de conferir o vídeo, escreva o título, descrição e tags que serão publicados no evento.")}
              </p>
            </div>
            <UploadFormView
              title={draft.title}
          ...
          <div className="sticky bottom-4 z-20 flex flex-wrap items-center gap-3 rounded-2xl border border-border/70 bg-card/90 p-3 shadow-[0_18px_60px_color-mix(in_oklab,var(--background)_45%,black_28%)] backdrop-blur-xl">
            <div className="min-w-0 flex-1 text-sm text-muted-foreground">
              {canPreparePublish
                ...
                : t("youtube_publish_missing_required", "Importe uma URL válida e informe um título para habilitar a publicação futuramente.")}
            </div>
            <ButtonWithLoader onClick={handlePublish} isLoading={isPending} disabled={!canPreparePublish || Boolean(publishedState)}>
              {t("Publish", "Publicar")}
