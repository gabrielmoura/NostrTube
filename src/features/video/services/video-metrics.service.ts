...
export async function getVideoZapMetrics(ndk: NDK, event: NDKEvent): Promise<VideoZapMetrics> {
  const zapEvents = deduplicateById(
    await fetchEventsCached(ndk, buildReferenceFilters(9735, event), {
      mode: 'parallel',
      closeOnEose: true,
    }),
  )
...
export async function getVideoCommentMetrics(ndk: NDK, event: NDKEvent): Promise<VideoCommentMetrics> {
  const commentEvents = deduplicateById(
    await fetchEventsCached(ndk, buildReferenceFilters(NDKKind.Text, event), {
      mode: 'parallel',
      closeOnEose: true,
    }),
  )
