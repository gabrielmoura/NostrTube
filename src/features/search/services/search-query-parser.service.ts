...
export function buildSearchStateFromInput(
  input: string,
  existing: Pick<eventSearchType, 'author' | 'lang' | 'tag'> = {},
) {
  ...
  return {
    ...
    chips: [
      ...
      ...((langChip?.value ?? existing.lang)
        ? [
            {
              type: 'lang' as const,
              value: langChip?.value ?? existing.lang!,
              label: `lang:${langChip?.value ?? existing.lang}`,
            },
          ]
        : []),
      ...((authorChip?.value ?? existing.author)
        ? [
            {
              type: 'author' as const,
              value: authorChip?.value ?? existing.author!,
              label: `author:${authorChip?.value ?? existing.author}`,
            },
          ]
        : []),
    ],
...
export function buildInputFromSearchState(
  searchParams: Pick<eventSearchType, 'search' | 'author' | 'lang' | 'tag'>,
): string {
  const tags = Array.isArray(searchParams.tag) ? searchParams.tag : searchParams.tag ? [searchParams.tag] : []
  const pieces = [
    searchParams.search,
    ...tags.map((tag) => `tag:${tag}`),
    searchParams.lang ? `lang:${searchParams.lang}` : '',
