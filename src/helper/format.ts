...
export function getNameToShow(user: {
  ...
  return user.profile?.displayName ?? user.profile?.name ?? truncateText(user.npub)
}
...
export function getVideoDetails(event: NDKEvent): VideoDetails {
  ...
  return {
    ...
    publishedAt: parseInt(
      getTagValues('published_at', event.tags)[0] ?? event.created_at?.toString() ?? unixTimeNowInSeconds().toString(),
    ),
    thumbnail:
      ...
      (url.includes('youtu')
        ? `http://i3.ytimg.com/vi/${
            url.includes('/youtu.be/') ? url.split('youtu.be/').pop() : url.split('?v=').pop()
          }/hqdefault.jpg`
        : ''),
    title: getTagValue('title', event.tags) ?? 'Untitled',
    summary: getTagValues('summary', event.tags) ?? getTagValues('about', event.tags) ?? (event.content as string),
  }
}
...
export function getTwoLetters(user: {
  ...
  if (user.profile) {
    if (user.profile.displayName) {
      ...
      const secondLetter = user.profile.displayName.split(' ')[1]?.at(0) ?? user.profile.displayName.at(1) ?? ''
      return firstLetter + secondLetter
    }
    if (user.profile.name) {
      const firstLetter = user.profile.name.at(0)
      const secondLetter = user.profile.name.split(' ')[1]?.at(0) ?? user.profile.name.at(1) ?? ''
      return firstLetter + secondLetter
    }
  }
 ...
 */
export function deduplicateParameterizedEvents(events: Set<NDKEvent> | NDKEvent[], targetKind: NDKKind): Set<NDKEvent> {
