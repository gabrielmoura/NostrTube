...
function applyThumbnailToDraft(
...
export function usePublishVideo() {
  ...
  const resolveThumbnailBeforePublish = async (snap: Partial<VideoMetadata>): Promise<Partial<VideoMetadata>> => {
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : t('thumbnail_generation_failed', 'Could not generate a thumbnail automatically.')
    ...
    if (isTemporaryUrl(thumbnailUrl)) {
      throw new Error(
        t(
          'temporary_thumbnail_url_error',
          'Thumbnail is still local. Upload it or use a public URL before publishing.',
        ),
      )
    }
