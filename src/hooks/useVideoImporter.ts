...
export function useVideoImporter() {
  ...
  const importFromEvent = async (eventString: string) => {
    ...
    try {
      ...
      let _data: nip19.AddressPointer | nip19.EventPointer
      let _type: 'naddr' | 'nevent'
      try {
        ...
        _data = decoded.data as nip19.AddressPointer | nip19.EventPointer
        _type = decoded.type
      } catch (_e) {
      ...
      const imeta = {
        ...
        fallback:
          primaryVariant?.candidates.slice(1).map((candidate) => candidate.url) ?? getTagValues('fallback', event.tags),
      ...
      setVideoUpload({
        ...
        imetaAudioTracks: assetSet.audioTracks.map(
          (track) =>
            ({
              url: track.candidates[0]?.url,
              m: track.mimeType,
              l: track.language
                ? `${track.language}${track.isOriginalVersion ? ' ISO-639-1 ov' : ' ISO-639-1'}`
                : undefined,
    ...
    if (ndk && mimeType?.startsWith('video/') && thumbnailGenerationMode === 'local') {
      try {
        const generated = await generateVideoThumbnailFromUrl(
          url,
          url.split('/').pop()?.split('?')[0] || 'external-video',
        )
