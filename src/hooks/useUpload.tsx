...
const useUpload = (props?: UploadProps) => {
  ...
  const uploadImage = async (file: File, folderName?: string) => {
    ...
    try {
      ...
      if (result.ok) {
        setStatus("success");
        const fileUrl = `${import.meta.env.NEXT_PUBLIC_S3_BUCKET_URL}/${fileName}`;
        setFileUrl(fileUrl);
  ...
  const ImagePreview = ({ className }: { className?: string }) => {
    ...
    return (
      <div className={cn("relative overflow-hidden rounded-xl", className)}>
        ...
        {status === "uploading" && (
          <button
            className="center absolute left-1 top-1 rounded-full bg-foreground bg-opacity-70 p-1 text-background hover:bg-opacity-100">
            <Spinner />
          </button>
        )}
        {status === "success" && (
