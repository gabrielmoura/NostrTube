...
const getConfig = (): AppConfig => {
  const relaysString = import.meta.env.PROD ? import.meta.env.VITE_NOSTR_RELAYS : import.meta.env.VITE_NOSTR_DEV_RELAYS
...
function generateAltTag(identifier: string, pubkey: string, config: AppConfig): string[] {
  ...
  return ['alt', `This is a video event and can be viewed at ${config.rootDomain}/v/${naddr}`]
}
...
export function generateVideoTags(props: GenTagsProps): string[][] {
  ...
  tags.push(['d', dTagValue], ['title', title], ['summary', summary || ''], ['published_at', nostrNow().toString()])
