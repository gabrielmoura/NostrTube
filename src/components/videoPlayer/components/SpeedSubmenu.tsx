...
export function SpeedSubmenu() {
  ...
  return (
    <Menu.Root>
      ...
      >
        <span className="font-medium">Velocidade:</span> <span className="opacity-80">{hint}</span>
      </Menu.Button>

      <Menu.Content className="mt-2 w-full rounded-md border border-white/10 bg-neutral-900/95 shadow-lg backdrop-blur-md p-1 text-sm text-white/90">
        <Menu.RadioGroup value={options.selectedValue} className="flex flex-col">
          {options.map(({ label, value, select }) => (
            ...
            >
              ...
              {options.selectedValue === value && <CheckIcon className="w-4 h-4 text-sky-400" />}
            </Menu.Radio>
          ))}
        </Menu.RadioGroup>
      </Menu.Content>
