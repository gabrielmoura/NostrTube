...
import { generatePrivateKey, KeyPair } from "@/helper/verify_event.ts";
...
export function LoginModal({
  ...
  return (
    <Dialog>
      ...
      <DialogContent className={cn("flex flex-col gap-6", className)}>
        {login ? <LoginContent setLogin={setLogin} className={className} {...props} /> :
          <Register setLogin={setLogin} className={className} {...props} />}
      </DialogContent>
    </Dialog>
  );
}
...
function LoginContent({ setLogin }: AuthProps | undefined) {
  ...
  if (kP) {
    return <div {...props}>
      <div className="flex flex-col items-center gap-2">
        ...
        <DialogTitle className="text-xl font-bold">Your Account is Ready!</DialogTitle>
        <DialogDescription className="text-center text-sm">
