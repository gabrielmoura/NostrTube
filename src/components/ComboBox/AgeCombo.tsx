    ...
    <Card className="w-full max-w-sm">
      ...
      <CardContent className="p-4 pt-0">
        <div className="flex flex-col gap-2">
          <Label className="text-xs text-muted-foreground">{t('age.combo.helper')}</Label>
          ...
          <div className="flex items-center gap-2">
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  <span className={cn('truncate', !selectedOption && 'text-muted-foreground')}>
                    {selectedOption ? t(`age.options.${selectedOption.id}.label`) : placeholder}
                  </span>
              ...
              <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" align="start">
                <Command>
                  <CommandInput placeholder={t('age.combo.search')} className="h-9" />
                  <CommandEmpty>{t('age.combo.empty')}</CommandEmpty>
                    ...
                    {options.map((option) => (
                      ...
                      >
                        <div className="flex flex-col">
                          <span className="font-medium">{t(`age.options.${option.id}.label`)}</span>
