...
export default function ButtonUpload({ children, url, setUrl, accept }: ButtonUploadProps) {
  ...
  const handleUploadFile = useCallback(async () => {
    ...
    try {
      const imeta = await blossom.upload(fileToUpload, {
        // Usa fileToUpload
  ...
  if (isLoading && typeof uploadProgress === 'number') {
    // Verifica se uploadProgress é um número
    return (
      <Card
        className={cn(
  ...
  return (
    <div className="flex gap-2 ">
      <div className="flex flex-col items-center gap-4">
        <div
          {...getRootProps({
            className: 'dropzone text-center cursor-pointer hover:border-blue-500 transition-colors',
          })}
        >
          <input {...getInputProps()} />
          {fileToUpload ? (
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm text-gray-500">
                {t('File_selected_click_upload_to_send', 'File selected. Click Upload to send.')}
              </p>
            </div>
          ) : (
            <>
              {children ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[100px]">
                  ...
                  <p className="text-gray-500">
                    {t('Drag_and_drop_file_here_or_click_to_select', 'Drag and drop file here, or click to select')}
                  </p>
                </div>
              )}
            </>
